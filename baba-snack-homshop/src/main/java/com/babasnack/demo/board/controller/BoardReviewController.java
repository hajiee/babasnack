...
public class BoardReviewController {
	...
	@GetMapping("/board/{bno}/board-read")
    public String reviewWriteForm(Model model, @PathVariable Long bno) {
		return "board/board-read";
    }

    // 리뷰 작성을 처리할 POST 메서드
	@PostMapping("/board/{bno}/board-read")
	public String reviewWrite(@PathVariable Long bno, @ModelAttribute WriteBR writeBR) {
        // 게시글 번호(bno)와 리뷰 내용(writeBR)을 사용하여 리뷰를 작성하는 코드 작성
        boardReviewService.addReview(writeBR);
